...
class ImageEditor:
    ...
    def create_menu(self):
        ...
        menubar.add_cascade(label="Gambar", menu=file_menu)
        file_menu.add_command(label="Input Gambar", command=self.open_image)

        edit_menu = Menu(menubar, tearoff=0)
        menubar.add_cascade(label="Perbaikan Gambar", menu=edit_menu)
        edit_menu.add_command(label="Penajaman Gambar", command=self.sharpen_image)
        edit_menu.add_command(label="Penghalusan Gambar", command=self.smooth_image)
    ...
    def create_widgets(self):
        ...
        Label(self.root, text="Sahrul", font=("Times New Roman", 12), pady=7).pack()
        Label(self.root, text="F55121040", font=("Times New Roman", 12), pady=7).pack()
...
if __name__ == '__main__':
    root = Tk()
    root.title("Aplikasi Perbaikan Citra Sahrul F55121040")
